			...
			if (file_exists ( "/var/www/podcasts/" . $_FILES ["file"] ["name"] )) {
				...
				if (isset ( $ResultEpisodeNum )) {
					...
					if (($_POST ['episode_date'] == '') || ($_POST ['episode_num'] == '') || ! isset ( $_POST ['episode_date'], $_POST ['episode_num'] )) {
						...
						$SHA1Upload = exec ( "fpcalc -length 600 -plain \"" . $_FILES ["file"] ["tmp_name"] . "\"" );
    ...
    <div class="container">
		...
		<form id="upload" action="?" method="post"
			...
			<div class="form-group">
				<input form="upload" class="form-control" name="file" type="file"
					id="file" required="true"><br /> <label for="episode_num">Episode
					Number</label> <input form="upload" class="form-control"
					type="number" id="episode_num" name="episode_num" min="0" step=".1"
					required="true"><br /> <label for="episode_date">Dropped</label> <input
					form="upload" class="form-control" type="date" id="episode_date"
					name="episode_date" required="true"><br /> <label
					for="episode_title">Title</label> <input form="upload"
					class="form-control" type="text" id="episode_title"
					name="episode_title" maxlength="256"><br /> <label
					for="episode_description">Description</label>
